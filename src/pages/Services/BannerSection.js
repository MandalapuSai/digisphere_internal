import React, { useEffect } from 'react';

const BannerSection = () => {
  useEffect(() => {
    // Initialize custom animations or libraries for the banner if needed
    const animateElements = () => {
      const elements = document.querySelectorAll('[data-anime]');
      elements.forEach((el) => {
        // Example: Check if 'data-anime' attribute is present and parse it
        const animationConfig = el.getAttribute('data-anime');
        if (animationConfig) {
          try {
            const parsedConfig = JSON.parse(animationConfig); // Parse the JSON configuration
            if (parsedConfig && Array.isArray(parsedConfig.translateY) && Array.isArray(parsedConfig.opacity)) {
              // Apply animation logic based on config
              el.style.transition = `all ${parsedConfig.duration || 600}ms ${parsedConfig.easing || 'easeOutQuad'}`;
              el.style.transform = `translateY(${parsedConfig.translateY[1]}px)`;  // Ensure valid index access
              el.style.opacity = parsedConfig.opacity[1];  // Ensure valid index access
            }
          } catch (e) {
            console.error('Error parsing animation config:', e);
          }
        }
      });
    };

    animateElements(); // Call animation on page load

    // Optional: Add event listener for scroll-based animation triggers
    window.addEventListener('scroll', animateElements);
    return () => window.removeEventListener('scroll', animateElements); // Cleanup on component unmount
  }, []);

  return (
    <section className="p-0 top-space-margin position-relative overflow-hidden">
      <div className="container-fluid p-0 h-100 position-relative">
        <div className="row g-0">
          <div
            className="col-xl-4 col-lg-5 d-flex justify-content-center align-items-center ps-10 xxl-ps-6 xl-ps-4 md-ps-4 sm-ps-0 position-relative order-2 order-lg-1"
            data-anime='{ "el": "childs", "translateY": [30, 0], "opacity": [0,1], "duration": 600, "delay":0, "staggervalue": 300, "easing": "easeOutQuad" }'
          >
            <div className="vertical-title-center align-items-center justify-content-center flex-shrink-0 w-75px sm-w-50px">
              <h1
                className="title fs-15 alt-font text-dark-gray fw-700 text-uppercase ls-1px text-uppercase d-flex w-auto align-items-center m-0"
                data-fancy-text='{ "opacity": [0, 1], "translateY": [50, 0], "filter": ["blur(20px)", "blur(0px)"], "string": ["our services"], "duration": 400, "delay": 0, "speed": 50, "easing": "easeOutQuad" }'
              >
                {/* Add dynamic or static content */}
                Our Services
              </h1>
            </div>
            <div className="border-start border-color-extra-medium-gray ps-40px sm-ps-20px position-relative z-index-9">
              <h2 className="text-dark-gray fw-600 alt-font outside-box-right-10 xl-outside-box-right-15 lg-outside-box-right-20 md-me-0 sm-mb-0 ls-minus-05px">
                We craft indelible digital experiences via the best services.
              </h2>
            </div>
          </div>
          <div className="col-xl-8 col-lg-7 position-relative one-half-screen order-1 order-lg-2 md-mb-50px">
            <div className="overflow-hidden position-relative">
              <div
                className="w-100"
                data-anime='{ "effect": "slide", "direction": "lr", "color": "#f7f7f7", "duration": 1000, "delay": 0 }'
              >
                <img
                  src="assets/img/best_services.jpg"
                  alt="Best Services"
                  className="w-100 liquid-parallax"
                  data-parallax-liquid="true"
                  data-parallax-position="top"
                  data-parallax-scale="1.05"
                />
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default BannerSection;
